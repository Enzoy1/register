...
class Block(nn.Module):
    def __init__(
        ...
        ema_open=False,  # 是否在 block 中启用 EMA 模块
        ema_factor=32,  # EMA 模块分组因子，要求 dim 能被 ema_factor 整除
    ):
        super().__init__()
        ...
        # 如果启用 EMA，则创建 EMA 模块，其 channels 设置为 dim
        if ema_open:
            self.ema = EMA(channels=dim, factor=ema_factor)
        else:
            self.ema = None
    ...
    def forward(self, x, give_attention=False):
        ...
        if self.ema is not None:
            B, N, C = x.shape  # 获取 batch_size, token 数量, 通道数

            # 判断特殊 token 数量：如果 (N - 2) 是完全平方数，则认为有2个特殊 token（例如CLS和distillation token），否则认为只有1个特殊 token
            special = 1
            if ((N - 2) ** 0.5) % 1 == 0:
                special = 2

            num_patches = N - special  # patch token 数量
            H = W = int(num_patches ** 0.5)  # 假设 patch 数量为完全平方数

            # 分离特殊 token和 patch tokens
            x_special, x_patches = x[:, :special, :], x[:, special:, :]
            # 调整 patch tokens 的形状为 (B, C, H, W)，以适应 EMA 模块的输入\n            x_patches = x_patches.permute(0, 2, 1).contiguous().view(B, C, H, W)
            # 经过 EMA 模块\n            x_patches = self.ema(x_patches)
            # 调整回原始格式 (B, num_patches, C)\n            x_patches = x_patches.view(B, C, -1).permute(0, 2, 1).contiguous()
            # 将特殊 token 与处理后的 patch tokens 拼接回 (B, N, C)\n            x = torch.cat([x_special, x_patches], dim=1)
        ...
        self.blocks = nn.Sequential(
            *[
                block_fn(
                    ...
                    ema_open=ema_open,   # 传入 ema_open 参数
                )
                for i in range(depth)
            ]
        )
        # 根据 ema_open 决定是否创建全局的 ema 模块（如果需要）
        self.ema = None
