...
class EMA(nn.Module):
    def __init__(self, dim, factor=32):
        ...
        print(">> ema open")
        self.groups = factor
        assert dim // self.groups > 0, "Dim must be divisible by groups"
        
        # 假设将序列视为2D特征图（需指定虚拟的H/W）
        self.assumed_h = 16  # 根据patch size调整，如224/16=14
        self.assumed_w = 16
        
        self.softmax = nn.Softmax(-1)
        self.gn = nn.GroupNorm(dim//self.groups, dim//self.groups)
        self.conv1x1 = nn.Conv2d(dim//self.groups, dim//self.groups, 1)
        self.conv3x3 = nn.Conv2d(dim//self.groups, dim//self.groups, 3, padding=1)

    def forward(self, x):
        B, N, D = x.shape
        H, W = self.assumed_h, self.assumed_w
        ...
        x_h = F.adaptive_avg_pool2d(group_x, (None, 1))
        x_w = F.adaptive_avg_pool2d(group_x, (1, None)).permute(0, 1, 3, 2)
        ...
        x11 = self.softmax(F.adaptive_avg_pool2d(x1, (1,1)).view(B*self.groups, -1, 1).permute(0,2,1))
        x12 = x2.view(B*self.groups, D//self.groups, -1)
        x21 = self.softmax(F.adaptive_avg_pool2d(x2, (1,1)).view(B*self.groups, -1, 1).permute(0,2,1))
        x22 = x1.view(B*self.groups, D//self.groups, -1)
        
        weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).view(B*self.groups, 1, H, W)
        out = (group_x * weights.sigmoid()).view(B, D, H, W)
        
        # 转换回3D格式 [B, N, D]
        return out.reshape(B, D, -1).transpose(1, 2)
...
class Block(nn.Module):
    def __init__(
        ...
        ema_enabled=False,  # 添加 ema_enabled 参数
    ):
        super().__init__()
        ...
        # EMA
        self.ema = EMA(dim) if ema_enabled else nn.Identity()
