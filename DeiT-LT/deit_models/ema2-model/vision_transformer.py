...
from deit_models.ema import EMA
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        ema_open=False,  # 是否启用 patch 级 EMA
        ema_factor=32,  # EMA 分组因子
    ):
        """
        Args:
        ...
        # 根据 ema_open 决定是否创建 EMA 模块
        self.ema = EMA(channels=embed_dim, factor=ema_factor) if ema_open else None
    ...
    def forward_features(self, x):
        ...
        if self.ema is not None:
            Bp, Np, Cp = x.shape
            Sp = int(Np ** 0.5)
            x_p = x.permute(0, 2, 1).contiguous().view(Bp, Cp, Sp, Sp)
            x_p = self.ema(x_p)
            x = x_p.view(Bp, Cp, Np).permute(0, 2, 1)  # [B, N, C]
